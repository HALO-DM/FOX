...
def main():
    ...
    t_sim0 = time.time()
    seeds = np.linspace(11,11,1, dtype = int)
    power_values = np.linspace(0,0,1)
    baselines = np.linspace(2,2,1)
        ...
        for key in baselines:
            for seed in seeds:
                ...
                if inj["enabled"]:
                    ...
                    ax = AxionParams(f_axion_hz=float(f_ax), sigma_hz=s_ax, total_power=power)
                ...
                specs, fper, rf, rf_map = simulate_spectra(
                    ...
                    tune_step_bins=sim["tune_step_bins"], rng_seed = seed,
                    noise_sigma=sim["noise_sigma"], axion=ax, baseline_key=key
                )
                ...
                try:
                    int_of_max = np.argmax(zvals[cands])
                    recovered_axion = freqs_r[cands[int_of_max]]
                except ValueError: 
                    recovered_axion = 0
                if recovered_axion >= f_ax - f_ax*0.00000175 and recovered_axion <= f_ax + f_ax*0.00000175 and zvals[cands[int_of_max]] >= 1.7:
                    recovery_count += 1
                    print(f"Axion Recovered at seed {seed}")

                time_1 = time.time()

                print(f"Time for run {counter} of {len(power_values) * len( baselines) * len(seeds)}: ", round(time_1 - time_0, 2))
                print("Total Runtime so far: ",round(time_1 - t_sim0,2), "\n")

        power_array = np.append(power_array, recovery_count)
    print(power_array)

    y_vals = (power_array / (len(baselines) * len(seeds))) * 100

    with (run_dir/"power.csv").open("w") as fh:
        fh.write(f"power,count % (total = {len( baselines) * len(seeds)})\n")
        for f,g in zip(power_values, y_vals):
            if np.isfinite(g): fh.write(f"{f},{g}\n")
    ...
    plt.text(
        power_values.min() + 1, threshold + 0.6,
    ...
    plt.tight_layout()
    plt.savefig(run_dir/"Power Values.png", dpi=150, bbox_inches='tight')
    plt.close()
