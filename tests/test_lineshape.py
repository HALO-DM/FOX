...
def test_shm_speed_pdf_normalizes():
    ...
    assert np.all(p >= 0)
    assert np.isclose(np.trapezoid(p, v), 1.0, rtol=1e-6, atol=1e-8)
...
def test_extreme_params_do_not_crash():
    ...
    assert np.isclose(np.trapezoid(p, v), 1.0, rtol=1e-5)
...
def test_template_behavior_near_f0():
    ...
    # 2) Below f0 should be ~0
    assert np.allclose(T[:c], 0.0, atol=1e-12)

    # 3) Peak should occur within the first few bins above f0,
    #    then generally decrease (allow tiny numerical wiggles).
    right = T[c+1:]
    imax = int(np.argmax(right))
    assert imax <= 2  # peak within first 2 bins above f0 is acceptable

    post = right[imax:]
    diffs = np.diff(post)
    assert np.count_nonzero(diffs > 1e-10) <= 1  # at most one micro-uptick
